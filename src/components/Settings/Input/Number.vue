    ...
    <label class="w-full flex flex-col justify-between items-start">
        ...
        <div class="w-full flex flex-row gap-2">
            <input type="number" v-model="inputValue" :placeholder="defaultString" :aria-label="label" :min :max
                @keyup.enter="updateValue" class="w-full p-2 rounded-lg ring-1 ring-base-400 hover:ring-base-300 outline-base-300 outline-0 focus:outline-2 transition-all duration-dynamic" />
            <div class="w-fit p-2 rounded-lg text-center aspect-square bg-primary hover:bg-base-300 text-base-700 cursor-pointer" @click="updateValue">
