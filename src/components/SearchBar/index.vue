...
<template>
    <Transition name="open-search">
        <div v-if="isShowingSearchMenu" class="absolute box-border flex items-top pt-16 justify-center top-0 left-0 w-full h-svh bg-black/50 z-99" @click.self="isShowingSearchMenu = false">
            <div class="flex flex-col gap-2 w-[calc(100dvw-1rem)] max-w-xl h-min max-h-2/3 p-2 bg-base-700 rounded-xl border-base-400 border-2 shadow-md shadow-base-950">
                <div class="flex flex-row gap-2">
                    <input
                        ...
                        v-model="searchQuery" 
                        class="p-3 w-full outline-none border-base-400 rounded-lg focus:border-base-500 border-2">
                    
                    <input type="checkbox" id="toggle-search-filters" v-model="isShowingFiltersMenu" class="sr-only">
                    <label for="toggle-search-filters" class="aspect-square bg-primary text-base-900 hover:bg-base-400 p-2 flex items-center justify-center rounded-lg cursor-pointer"
                        :class="{ 'hover:bg-base-400! text-base-200 bg-base-500!': isShowingFiltersMenu }">
                ...
                <div v-if="isShowingFiltersMenu" class="flex flex-row items-center gap-2 pt-5 pb-6 overflow-x-auto">
                    ...
                    <select id="search-sort-type" v-model="sortType" class="bg-base-600 p-2 border-2 border-base-500 rounded-lg">
                    ...
                    <select id="search-sort-direction" v-model="reverseSort" class="bg-base-600 p-2 border-2 border-base-500 rounded-lg">
                    ...
                    <select id="search-user-filter" v-model="userFilter" class="bg-base-600 p-2 border-2 border-base-500 rounded-lg">
