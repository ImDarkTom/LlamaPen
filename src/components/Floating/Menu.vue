...
import { inject, onBeforeUnmount, onMounted, provide, ref, toRef, useId } from 'vue';
...
<template>
    <div v-click-outside="handleClickOutside">
        <div
            :class="{
                'flex flex-row items-center gap-1 w-max select-none cursor-pointer rounded-lg transition-all duration-dynamic hover:bg-base-700 active:scale-98 h-10 p-2 box-border text-sm font-medium':
                    !unstyledButton,
                'group active': isOpened,
                'bg-base-600!': isOpened && !unstyledButton,
            }"
        ...
        <Teleport to="body">
            <Transition
                ...
                <div
                    ...
                    :class="[
                        {
                            'bg-base-700 p-1.5 flex flex-col gap-2 rounded-lg max-w-[100dvw-3rem] w-full shadow-elevation-4':
                                !unstyledMenu,
                        },
                        unstyledMenu ? '' : 'sm:w-96',
                    ]"
                    :style="menuPosition">
                    <slot name="menu" />
                </div>
