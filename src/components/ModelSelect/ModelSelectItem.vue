	...
	<li class="relative group flex flex-col cursor-pointer p-3 hover:bg-primary-400 transition-colors duration-75 rounded-lg overflow-x-hidden"
	    ...
	    <div class="flex flex-row items-center justify-between">
			<div class="flex flex-row">
				<span
					class="text-md font-semibold text-ellipsis whitespace-nowrap overflow-hiddefont-bold"
					:class="{'text-yellow-50': model.llamapenMetadata?.premium}"
					:title="model.name">{{ model.name }}</span>
				<span v-if="model.llamapenMetadata?.premium" class="bg-yellow-400/25 rounded-sm ring-1 ring-yellow-400 size-5 ml-2 p-0.5">
					<AiFillStar class="text-yellow-400 size-4" />
				</span>
			</div>
			<div class="flex flex-row">
				<span v-if="model.capabilities?.includes('vision')" class="bg-green-400/25 rounded-sm ring-1 ring-green-400 size-5 ml-2 p-0.5">
					<AiOutlineEye class="text-green-400 size-4" />
				</span>
				<span v-if="model.capabilities?.includes('tools')" class="bg-blue-400/25 rounded-sm ring-1 ring-blue-400 size-5 ml-2 p-0.5">
					<AiOutlineTool class="text-blue-400 size-4" />
				</span>
				<span v-if="model.capabilities?.includes('search')" class="bg-violet-400/25 rounded-sm ring-1 ring-violet-400 size-5 ml-2 p-0.5">
